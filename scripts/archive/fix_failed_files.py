...
using the configured model with reduced context to avoid the same issues.
"""
...
Response:"""
    ...
    try:
        response = client.chat.completions.create(
            ...
            **({"max_completion_tokens": 1500} if os.getenv('DEFAULT_OPENAI_MODEL', 'gpt-5-mini').startswith('gpt-5') else {"max_tokens": 1500}),  # Reduced from 2000
            temperature=0.7
        )
...
def regenerate_failed_file(failed_file, products, prompts):
    ...
    print(f"  🤖 Regenerating with {os.getenv('DEFAULT_OPENAI_MODEL', 'gpt-5-mini')} and reduced context...")

...
def extract_prompt_details(content):
    ...
    Normalizes the model to an API-ready identifier (e.g., "gpt-5") if a GPT token is present.
    """
    import re as _re
    ...
    for i, line in enumerate(lines):
        if line.startswith('# '):
            ...
            model_raw = line.replace('**Model:**', '').strip()
        elif 'Kick-Off Prompt:' in line:
            # Find the actual prompt (usually in quotes on the next few lines)
            for j in range(i + 1, min(i + 10, len(lines))):
                candidate = lines[j].strip()
                if candidate.startswith('"') and candidate.endswith('"'):
                    kick_off_prompt = candidate[1:-1]  # Remove quotes
                    break

    # Normalize model: prefer first token that looks like a gpt model id
    model = model_raw
    match = _re.search(r"\b(gpt[\w-]*)\b", model_raw, _re.IGNORECASE)
    if match:
...
def call_llm(prompt, product_context, previous_outputs, model_preference=None):
    """Call the LLM with the prompt and context"""
    global client
    
    # Use model from environment variable if not specified
    if model_preference is None:
        model_preference = os.getenv('DEFAULT_OPENAI_MODEL', 'gpt-5-mini')
...
Response:"""
    ...
    try:
        ...
        actual_model = model_preference if model_preference.startswith('gpt') else "gpt-4o-mini"
        create_params = {
            "model": actual_model,
            "messages": [
